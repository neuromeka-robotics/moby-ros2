...
class MobyROSConnector(Node):
    ...
    def rotate_by(self, delta_deg):
        ...
        rot_cur = self.ecat.get_servo_tx(slave_idx)[2] / 1000 # actual pos
    ...
    def camera_elevator_callback(self, msg: CameraHeight):
        ...
        if sync_mode:
            # self.ecat.set_md_rx_pdo(slave_idx, 0x1f, 1, target_pos_cnt, 0, 0)
            self.ecat.set_servo_rx(slave_idx, 0x1f, 1, target_pos_cnt, 0, 0)
        else:
            # self.ecat.set_md_rx_pdo(slave_idx, 0x3f, 1, target_pos_cnt, 0, 0)
            self.ecat.set_servo_rx(slave_idx, 0x3f, 1, target_pos_cnt, 0, 0)
            time.sleep(0.01)
    ...
    def rail_sensor_publish_callback(self):
        if self.ecat is not None:
            ...
            di = self.ecat.get_di(0) # specific list
            left_sensor = di[2]
            right_sensor = di[3]
        else:
            left_sensor = 0
